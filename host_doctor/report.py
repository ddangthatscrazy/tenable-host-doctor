...
<head>
    ...
    <title>Tenable Host Doctor Report - {report.host_ip}</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">Diagnostic Analysis for {report.host_ip}</div>
            <div class="meta">
                <div>Scan: {report.scan_name}</div>
                <div>Generated: {report.generated_at.strftime("%Y-%m-%d %H:%M:%S")}</div>
                <div>Source: {report.nessus_file}</div>
            </div>
        </div>
